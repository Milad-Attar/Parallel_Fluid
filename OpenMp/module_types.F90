module module_types
  ...
  implicit none
  real(wp), public :: t3, t4, t5, t6, t7, rate
  private
  ...
  subroutine update(s2,s0,tend,dt)
    ...
    do ll = 1, NVARS
      do k = 1, nz
        do i = 1, nx
          !!!take one variable, take one cell(i,k), compute the updated state using the old state s0 and tendency, store the result in s2
          s2%mem(i,k,ll) = s0%mem(i,k,ll) + dt * tend%mem(i,k,ll)
        end do
      end do
    end do
    call system_clock(tstop,rate)
    t7 = t7 + real(tstop - tstart, wp)/rate
  end subroutine update

  subroutine xtend(tendency,flux,ref,atmostat,dx,dt)
    implicit none
    !!! tendency is an object of the atmospheric_tendency type and we have the same for flux, ref, and atmostat
    ...
    !!! we have nx+1 because we have an extra face in x dirextion
      do i = 1, nx+1
        ...
        do ll = 1, NVARS
          ...
          !stencil(1:STEN_SIZE) = atmostat%mem(i-hs-1+1:i-hs-1+STEN_SIZE,k,ll)

          !stencil(1) = atmostat%mem(i-hs-1+1,k,ll)
          !stencil(2) = atmostat%mem(i-hs-1+2,k,ll)
          !stencil(3) = atmostat%mem(i-hs-1+3,k,ll)
          !stencil(4) = atmostat%mem(i-hs-1+4,k,ll)

          !!!reconstruct values at the face using stencils
        ...
        p = c0*(r*t)**cdocv
        !!!Flux: flow of a quantity through a surface per unit time.
        !!!compute fluxes from the vals
    ...
    call system_clock(tstop,rate)
    t3 = t3 + real(tstop - tstart, wp)/rate

    call system_clock(tstart1)
    !$omp parallel do collapse(3)
    do ll = 1, NVARS
      do k = 1, nz
        do i = 1, nx
        !!! for each cell, along x direction we already calculate the flux and it's not necessary to calculate inside the loop, so we don't have race codition
          tendency%mem(i,k,ll) = &
              -( flux%mem(i+1,k,ll) - flux%mem(i,k,ll) ) / dx
        end do
      end do
    end do
    call system_clock(tstop1,rate)
    t4 = t4 + real(tstop1 - tstart1, wp)/rate
  ...
  subroutine ztend(tendency,flux,ref,atmostat,dz,dt)
    ...
    do k = 1, nz+1
      do i = 1, nx
        do ll = 1, NVARS
          ...
          !stencil(1:STEN_SIZE) = atmostat%mem(i,k-hs-1+1:k-hs-1+STEN_SIZE,ll)

          !stencil(1) = atmostat%mem(i,k-hs-1+1,ll)
          !stencil(2) = atmostat%mem(i,k-hs-1+2,ll)
          !stencil(3) = atmostat%mem(i,k-hs-1+3,ll)
          !stencil(4) = atmostat%mem(i,k-hs-1+4,ll)
    ...
    call system_clock(tstop,rate)
    t5 = t5 + real(tstop-tstart, wp)/rate
      ...
      do k = 1, nz
        do i = 1, nx
          tendency%mem(i,k,ll) = &
              ...
              !!! we add if condition because just for z direction we have gravity, so only for velocity in z we have the below line
          if (ll == I_WMOM) then
    ...
    call system_clock(tstop1,rate)
    t6 = t6 + real(tstop1 - tstart1, wp)/rate
  ...
  subroutine exchange_halo_x(s)
    ...
    integer :: k, ll
    do ll = 1, NVARS
    !!! we need a loop in z direction because we want to exchange the halos for each cell in x direction
      do k = 1, nz
      !!! we choose 4 points because we want to have higher approximation of the flux
