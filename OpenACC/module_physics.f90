module module_physics
  ...
  subroutine total_mass_energy(mass,te)
    ...
    te = 0.0_wp
    call system_clock(tstart)
    !!!move data to GPU and then move the result (mass, te) to CPU
    !!!start a data region
    !!!oldstat%mem -> oldstat%dens, oldstat%umom, oldstat%wmom, oldstat%rhot
    !$acc data copyin(oldstat%mem(1:nx,1:nz,1:NVARS), ref%density(1:nz), ref%denstheta(1:nz))
    !$acc data copy(mass, te)
